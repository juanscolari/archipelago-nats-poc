...
async function main() {
  ...
  try {
    ...
    const registerSubscribe = (subject: string, cb: Function, spread?: boolean) => {
      const subscription = nc.subscribe(subject)
      ;(async () => {
        for await (const message of subscription) {
          try {
            if (message.data.length) {
              const data = jc.decode(message.data) as any
    ...
    const onIslandUpdates = async (updates: IslandUpdates) => {
      nc.publish("archipelago.subscribeToUpdates", jc.encode(updates))
      console.log(`archipelago.subscribeToUpdates: ${JSON.stringify(updates)}`)
    }
    controller.subscribeToUpdates(onIslandUpdates.bind(controller))

    // Subscribe
    registerSubscribe("archipelago.setPeersPositions", controller.setPeersPositions, true)
    registerSubscribe("archipelago.clearPeers", controller.clearPeers, true)
